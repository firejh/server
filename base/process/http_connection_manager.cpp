...
#include "component/other_tools.h"
...
void HttpConnectionManager::epollerEventHandler(int32_t socketFD, net::Epoller::Event event)
{
    ...
    try
    {
        ...
        case net::Epoller::Event::read:
            {
                while(conn->tryRecv())
                {
                    //这里操作完就结束，函数不能有阻塞操作，也不支持异步操作，适用于获取内存数据然后立刻返回
                    //如果需要避免上述限制，需要加上缓存池，类似apache等，开一个新的线程来执行函数，这样函数内部可以阻塞。这样的话可能需要再加一个容器来维护正在使用的线程，函数结束后线程还给线程池，连接要关闭
        ...
        case net::Epoller::Event::write:
            {
                component::LockFreeCircularQueueSPSC<net::Packet::Ptr>::Ptr queue = connHolder->sendQueue;
...
bool HttpConnectionManager::sendPacket(ConnectionHolder::Ptr connHolder, net::Packet::Ptr packet)
{
    ...
    if(connHolder->sendQueue == nullptr)
    {
        ...
        connHolder->sendQueue = std::make_shared<component::LockFreeCircularQueueSPSC<net::Packet::Ptr>>(9);
    }
