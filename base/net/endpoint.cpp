...
{
    std::vector<std::string> items = component::splitString(str, ".");
    items.resize(4);

    bytes[0] = component::fromString<int>(items[0]);
    bytes[1] = component::fromString<int>(items[1]);
    bytes[2] = component::fromString<int>(items[2]);
    bytes[3] = component::fromString<int>(items[3]);
}
...
{
    return ip.toString() + ":" + component::toString(port);
}

void Endpoint::fromString(const std::string& str)
{
    //std::vector<std::string> strs = component::splitString(str, ":");
    std::vector<std::string> strs;
    component::splitString(&strs, str, ":");
    if(strs.size() < 2)
...
//#endif
    ...
    ip = component::fromString<IpV4>(strs[0]);
    port = component::fromString<uint16_t>(strs[1]);
}

void Endpoint::appendToString(std::string* str) const
{
    component::formatAndAppend(str, "{}:{}", ip, port);
}
